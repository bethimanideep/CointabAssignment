...
const UserPosts = () => {
    ...
    fetch(`https://jsonplaceholder.typicode.com/posts?userId=${userId}`)
      ...
      .then((data) => {
        ...
        const checkPresencePromises = data.map((post) =>
          fetch(`https://dull-ruby-dugong-cap.cyclic.appcheckPost/${userId}/${post.id}`)
            .then((response) => response.json())
            .then((result) => result)
            .catch((error) =>
  ...
  const handleBulkAdd = () => {
    ...
    if (postsToAdd.length > 0) {
      fetch(`https://dull-ruby-dugong-cap.cyclic.appbulkAddPosts/${userId}`, {
        method: 'POST',
        headers: {
  ...
  const handleDownloadExcel = () => {
    ...
    fetch(`https://dull-ruby-dugong-cap.cyclic.appdownloadExcel/${userId}`)
